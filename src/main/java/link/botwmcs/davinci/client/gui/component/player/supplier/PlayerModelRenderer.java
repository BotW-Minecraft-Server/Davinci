package link.botwmcs.davinci.client.gui.component.player.supplier;

import com.mojang.blaze3d.vertex.PoseStack;
import com.mojang.blaze3d.vertex.VertexConsumer;
import com.mojang.math.Axis;
import link.botwmcs.davinci.client.gui.component.player.PlayerEntityModel;
import link.botwmcs.davinci.client.gui.component.player.supplier.layer.PlayerCapeLayer;
import link.botwmcs.davinci.client.gui.component.player.supplier.layer.PlayerEntityRenderLayer;
import link.botwmcs.davinci.client.gui.component.player.supplier.layer.PlayerParrotLayer;
...
public class PlayerModelRenderer extends PlayerRenderer {
    public static final EntityModelSet MODEL_SET = Minecraft.getInstance().getEntityModels();
    public static final EntityRendererProvider.Context RENDER_CONTEXT = new EntityRendererProvider.Context(Minecraft.getInstance().getEntityRenderDispatcher(), Minecraft.getInstance().getItemRenderer(), Minecraft.getInstance().getBlockRenderer(), Minecraft.getInstance().getEntityRenderDispatcher().getItemInHandRenderer(), Minecraft.getInstance().getResourceManager(), MODEL_SET, Minecraft.getInstance().font);

    public final PlayerModelProperties properties;
    public final PlayerModelSupplier playerModel;

    public PlayerModelRenderer(boolean useSlimModel) {
        super(RENDER_CONTEXT, useSlimModel);
        this.properties = new PlayerModelProperties(useSlimModel);
        this.playerModel = new PlayerModelSupplier(RENDER_CONTEXT.bakeLayer(useSlimModel ? ModelLayers.PLAYER_SLIM : ModelLayers.PLAYER), useSlimModel, this.properties);
        this.addLayer(new PlayerParrotLayer(this, RENDER_CONTEXT.getModelSet(), this.properties));
        this.addLayer(new PlayerCapeLayer(this, this.properties));
    }
    ...
    private void setModelProperties() {
        PlayerModelSupplier playermodel = this.playerModel;
        if (this.properties.isSpectator()) {
    ...
    private static HumanoidModel.ArmPose getArmPose(PlayerModelProperties props, InteractionHand interactionHand) {
